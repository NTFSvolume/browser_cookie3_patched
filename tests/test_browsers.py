...
from .utils import BrowserName as B
...
class Test(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.__is_github_actions = bool(os.environ.get("GITHUB_ACTIONS", False))
        if cls.__is_github_actions:
            cls.__headless = True
        else:
            cls.__headless = bool(os.environ.get("RUN_HEADLESS", False))
        if cls.__headless:
    ...
    def __wait_for_cookies_to_be_detected(
        self, browser_func: _CookieExtractor, cookies_path: Optional[str], key_path: Optional[str], timeout: int
    ):
    ...
    def __setup_firefox_based(self, profile_containing_dir: str, binary_location: str) -> None:
        ...
        def extract_profile_tar():
            xz_file: str = tempfile.mktemp(suffix=".tar.xz")
        ...
        extract_profile_tar()
        profile_dir: str = os.path.join(profile_containing_dir, FIREFOX_BASED_PROFILE_DIR_NAME)
    ...
    @staticmethod
    def __call_browser_func(func: _CookieExtractor, cookies_path: Optional[str] = None, key_path: Optional[str] = None):
        if not key_path:
    ...
    def __setup_chromium_based(self, chrome_type, binary_location: str, driver_version=None):
        ...
        self.driver = webdriver.Chrome(
            service=ChromeService(ChromeDriverManager(version=driver_version, chrome_type=chrome_type).install()),
            options=options,
        )

    def __test_chromium_based(self, browser_func: _CookieExtractor, max_wait_seconds: int = 45):
    ...
    def __test_opera_based(self, browser_func: _CookieExtractor):
        cookie_path = ["Cookies"]
        key_path = None
        if sys.platform == "win32":
        ...
        try:
            ...
            if "os_crypt" in str(e) and sys.platform == "win32":
                ...
                logger.warning("it is a common issue with Opera-based browsers on Windows")
    ...
    def __setup_edge(self):
        ...
        options.binary_location = self.__binary_location.get(B.EDGE)
        options.add_argument(f"--user-data-dir={self.__get_data_dir()}")
        if self.__headless:
            options.add_argument("--headless=new")
            options.add_argument("--disable-gpu")
    ...
    def test_brave(self):
        self.__setup_chromium_based(ChromeType.BRAVE, self.__binary_location.get(B.BRAVE))
        self.__test_chromium_based(brave)

    def test_chromium(self):
        self.__setup_chromium_based(ChromeType.CHROMIUM, self.__binary_location.get(B.CHROMIUM))
        self.__test_chromium_based(chromium)

    def test_chrome(self):
        self.__setup_chromium_based(ChromeType.GOOGLE, self.__binary_location.get(B.CHROME))
    ...
    def test_firefox(self):
        ...
        binary_location = self.__binary_location.get(B.FIREFOX)
    ...
    def test_librewolf(self):
        ...
        cookie_path = os.path.join(profile_containing_dir, FIREFOX_BASED_PROFILE_DIR_NAME, "cookies.sqlite")
        binary_location = self.__binary_location.get(B.LIBREWOLF)

        self.__setup_firefox_based(profile_containing_dir, binary_location)
        self.__test_browser(librewolf, cookie_path)

    def test_opera(self):
        self.__setup_opera_based(self.__binary_location.get(B.OPERA))
    ...
    def test_opera_gx(self):
        self.__setup_opera_based(self.__binary_location.get(B.OPERA_GX))
    ...
    def test_vivaldi(self):
        ...
        driver_version = get_vivaldi_driver_version(self.__binary_location.get(B.VIVALDI), self.__is_github_actions)
        logger.info(f"Using chromedriver version {driver_version} for Vivaldi")
        self.__setup_chromium_based(ChromeType.GOOGLE, self.__binary_location.get(B.VIVALDI), driver_version)
